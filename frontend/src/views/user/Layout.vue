<template>
  <div class="layout">
    ...
    <van-tabbar v-model="active" active-color="#2563eb" inactive-color="#94a3b8" @change="onTabChange" class="custom-tabbar">
      ...
      <van-tabbar-item icon="comment-o" name="dish-request" :badge="myPendingCount || ''">求菜</van-tabbar-item>
...
async function loadMyPending() {
  try {
    ...
    myPendingCount.value = list.filter(r => r.status === 'PENDING').length
  } catch {}
}

onMounted(loadMyPending)
</script>

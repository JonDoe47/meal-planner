...
- **食材采购横幅**：管理后台 Dashboard 新增今日食材采购卡片，有家庭成员点餐后自动显示「共需 N 种食材」，一键跳转食材清单页，方便管理员采购
- **食材输入改为文本框**：菜品管理的食材编辑从标签+逐个添加改为多行文本框，支持每行一个或逗号分隔，AI 识别到食材后直接填入文本框可即时编辑
- **网页图标**：新增 `favicon.svg`（橙色主题餐盘+筷子），浏览器标签页现在有专属图标

**🔧 修复与优化**

- **收藏夹全量获取**：B 站收藏夹导入修复只取第 1 页（40 条）的问题，现在自动翻页拉取全部视频
- **批量导入自动封面**：AI 分析视频时同步下载封面到本地，不再依赖 B 站 CDN 直链（解决防盗链导致封面无法显示的问题）
